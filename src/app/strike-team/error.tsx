...
export default function StrikeTeamError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[BASE8HQ] Strike Team error:", error);
  }, [error]);
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4 relative overflow-hidden">
      ...
      >
        ...
        <span className="font-heading text-[0.6rem] tracking-[0.3em] text-destructive mb-3 inline-block">
          {"// Personnel Data Fault"}
        </span>
        <h1 className="font-heading text-2xl font-bold text-foreground mb-3">
          Team Dossiers Lost
        </h1>
        <p className="text-muted-foreground text-sm leading-relaxed mb-6">
          Strike Team dossiers could not be loaded. Attempt to recover the data
          or return to a safe position.
        </p>
        {error?.digest && (
